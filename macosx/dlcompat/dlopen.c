 ...
 */
 ...
 /*
  * This sourcecode was downloaded straight from the Darin CVS repisitory.
  * All changes are marked with the comment "// <AWE> ..."
  */
...
static
enum DYLD_BOOL							// <AWE> changed "bool" to "DYLD_BOOL".
NSMakePrivateModulePublic(
NSModule module)
{
    static enum DYLD_BOOL (*p)(NSModule module) = NULL;		// <AWE> changed "bool" to "DYLD_BOOL".
	...
	if(module == NULL){
	    NSLinkEditError(&NSLinkEditError_errorClass,
			    &NSLinkEditError_errorNumber,
    			    (const char **) &NSLinkEditError_fileName,		// <AWE> added const cast.
			    (const char **) &NSLinkEditError_errorString);	// <AWE> added const cast.
