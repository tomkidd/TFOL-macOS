...
void *	qglGetProcAddress (const char *theName, enum qglGetAdrMode theMode)
{
    ...
    {
        VID_Error (ERR_FATAL, "Failed to import a required OpenGL function!\n");
    }
...
//            break;
    ...
    myError = glGetError ();
	if (myError != GL_NO_ERROR)	// Knightmare: added separate output for this
		VID_Printf (PRINT_DEVELOPER, "QGL_SafeTexImage: OpenGL Error 0x%x\n", myError);
...
//    if (myError != GL_NO_ERROR || theWidth != myWidth)
	if (myWidth > theWidth)	// Knightmare: just do a debug output if texture is larger
		VID_Printf (PRINT_DEVELOPER, "QGL_SafeTexImage: Texture upload larger than expected.\nUpload width: %i, expected width: %i\n", myWidth, theWidth);	
	else if (myWidth < theWidth)	// Knightmare: only give this error if the texture is smaller
    {
        VID_Printf (PRINT_ALL, "QGL_SafeTexImage: Error: Out of texture RAM!\nPlease choose a lower texture quality or resolution.\n");
		VID_Printf (PRINT_DEVELOPER, "QGL_SafeTexImage: Texture width: %i, expected width: %i\n", myWidth, theWidth);	// Knightmare: added debug output
...
void GLimp_EnableLogging (qboolean theEnable)
{
    if (theEnable == true)
    {
        if (gQGLLogFile == NULL)
        {
            ...
            // open the log file:
            snprintf (myBuffer, MAX_OSPATH, "%s/gl.log", FS_Gamedir ()); 
