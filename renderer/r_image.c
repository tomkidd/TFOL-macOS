...
void R_CreatePNG (void) 
{
	...
	r_png_handle->colorType = PNG_COLOR_TYPE_RGB;
...
void R_LoadJPG (char *filename, byte **pic, int *width, int *height)
{
	...
	*width = cinfo.output_width; *height = cinfo.output_height;
	...
	// Free raw data buffer Jul 19 2012 Memory Leak fix.
    FS_FreeFile(rawdata);
...
image_t	*R_FindImage (char *name, imagetype_t type)
{
	...
	// Knightmare fix, 8-26-2012
	// fix up bad (backslash) image paths
    tmp = name;
    while ( *tmp != 0 )
    {
        if ( *tmp == '\\' )
