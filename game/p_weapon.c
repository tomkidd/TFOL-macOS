...
void weapon_shotgun_fire (edict_t *ent, qboolean altfire)
{
	...
	{
		if(!Q_stricmp(viewing->classname,"item_freezable") && range <= 40)   //bc 8-17-2012  increased hammer hit distance.
...
void launch_orangepeel ()
{
	...
	gi.setmodel (gib, "models/objects/orange/peel.md2");
	gib->solid = SOLID_NOT;

	VectorCopy(player->s.origin, gib->s.origin);
	gib->s.origin[2] += player->viewheight - 4;
	...
	VectorCopy(player->client->v_angle, randAng);
	...
	//add player velocity.
	if (player->client)
		VectorAdd (gib->velocity, player->velocity, gib->velocity);	
...
void Weapon_Railgun (edict_t *ent)
{
	...
	Weapon_Generic (ent,
		...
		141, //idle last
		142, //deactivate last
		pause_frames, fire_frames, weapon_railgun_fire);*/
	int LASTACTIVATEFRAME = 5;
	int IDLEFRAME = 5;
	int FIRSTFIREFRAME = 10;
	int LASTFIREFRAME = 79;
	...
	if (ent->client->weaponstate == WEAPON_ACTIVATING)
	{
		...
		{
			ent->client->weaponstate = WEAPON_READY;
			ent->client->ps.gunframe = IDLEFRAME;
			return;
		}
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		if (ent->client->ps.gunframe >= LASTFIREFRAME)
		{
			edict_t *player;	
			player = &g_edicts[1];	// Gotta be, since this is SP only
			...
			//done w/ firing.
			ent->client->weaponstate = WEAPON_DROPPING;
			gi.sound(ent, CHAN_ITEM, gi.soundindex("world/eat.wav"), 1, ATTN_NONE, 0);

			//remove the weapon.
			if (player->client->pers.inventory[ITEM_INDEX(FindItem("weapon_orange"))] > 0)
			{
				player->client->pers.inventory[ITEM_INDEX(FindItem("weapon_orange"))] = 0;
			}
		...
		{
			case 11:
				gi.sound(ent, CHAN_ITEM, gi.soundindex("world/orange1.wav"), 1, ATTN_NONE, 0);
				break;
			case 21:
				gi.sound(ent, CHAN_ITEM, gi.soundindex("world/orange2.wav"), 1, ATTN_NONE, 0);
				break;
			case 31:
				gi.sound(ent, CHAN_ITEM, gi.soundindex("world/orange3.wav"), 1, ATTN_NONE, 0);
				break;
			case 41:
				gi.sound(ent, CHAN_ITEM, gi.soundindex("world/orange4.wav"), 1, ATTN_NONE, 0);
				break;
			case 51:
				gi.sound(ent, CHAN_ITEM, gi.soundindex("world/orange1.wav"), 1, ATTN_NONE, 0);
				break;
	...
	if (ent->client->weaponstate == WEAPON_READY && ent->client->ps.gunframe == IDLEFRAME && ((ent->client->latched_buttons|ent->client->buttons) & BUTTONS_ATTACK))
	{
		ent->client->ps.gunframe = FIRSTFIREFRAME;
		ent->client->weaponstate = WEAPON_FIRING;
	}
}
...
//int *pause_frames, int *fire_frames, void (*fire)(edict_t *ent, qboolean altfire))
...
void Weapon_Camera(edict_t *ent)
{
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		if ( ((ent->client->latched_buttons|ent->client->buttons) & BUTTONS_ATTACK) && (ent->client->cardraising==0))
		{
			...
			{
				if(!Q_stricmp(tr.ent->classname,"item_photoitem"))
				{
