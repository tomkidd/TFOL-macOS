...
// USE - special actions taken when +use is pressed
	if(!client->use && (ucmd->buttons & BUTTON_USE))
	{
		...
		//BC close target_text if it is active.
		if(client->textdisplay)
		{
			Text_Close(ent);
			return;
		}
		...
		//else
		{
			...
			{
				//bc for the func_inspect.
				if (client->ps.fov < 90 && !Q_stricmp(viewing->classname,"func_inspect"))
				{
					client->ps.fov = 90;
					gi.sound (ent, CHAN_VOICE, gi.soundindex( "world/snap2.wav"), 1, ATTN_NONE, 0);
				}

				if(!Q_stricmp(viewing->classname,"monster_olive") && range <= 72)
					olive_action(viewing,ent);

				if(!Q_stricmp(viewing->classname,"monster_worker") && range <= 72)
					worker_action(viewing,ent);


				if(!Q_stricmp(viewing->classname,"item_doorknob") && range <= 72)
					doorknob_action(viewing,ent);

				if(!Q_stricmp(viewing->classname,"item_lever") && range <= 72)
					lever_action(viewing,ent);

				if(!Q_stricmp(viewing->classname,"item_wallbutton") && range <= 300)
					wallbutton_action(viewing,ent);


				if(!Q_stricmp(viewing->classname,"item_suitcase") && range <= 72)
					suitcase_action(viewing,ent);


				if(!Q_stricmp(viewing->classname,"item_pickable") && range <= 72)
					pickable_action(viewing,ent);

				if(!Q_stricmp(viewing->classname,"func_usebutton") && range <= 100)
				...
				if(!Q_stricmp(viewing->classname,"func_trainbutton") && (viewing->spawnflags & 1) && range <= 72)
					trainbutton_use(viewing,ent,ent);


				if (!Q_stricmp(viewing->classname,"item_bottle") && (range <= 100) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					bottle_use(viewing,ent,ent);


				if (!Q_stricmp(viewing->classname,"target_commentary") && (range <= 300) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					use_commentary(viewing,ent,ent);



				if (!Q_stricmp(viewing->classname,"func_button") && (range <= 300) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					button_use(viewing,ent,ent);

				if (!Q_stricmp(viewing->classname,"func_door") && (range <= 300) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					door_use(viewing,ent,ent);

				if (!Q_stricmp(viewing->classname,"func_inspect") && (range <= 300) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					use_inspect(viewing,ent,ent);


				if(!Q_stricmp(viewing->classname,"item_pickup") && (range <= 100) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					pickup_use(viewing,ent,ent);

				if(!Q_stricmp(viewing->classname,"item_bulletbox") && (range <= 100) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					bulletbox_use(viewing,ent,ent);

				if(!Q_stricmp(viewing->classname,"item_deck") && (range <= 100) && (ent->client->ps.stats[STAT_USEABLE] > 0))
					deck_use(viewing,ent,ent);


				if (!Q_stricmp(viewing->classname,"item_radio") && (range <= 100) && (ent->client->ps.stats[STAT_USEABLE] > 0))				
					radio_use(viewing,ent,ent);
				


				//gi.dprintf("name  %s    %f   \n", viewing->classname, range );
				...
				if(
					(
						(
							(
								(
									!Q_stricmp(viewing->classname,"func_door") ||								
									!Q_stricmp(viewing->classname,"func_button") 									 
								)
								&&
								(viewing->spawnflags & 2)
							)

							/*BC  This is a list of all Useable stuff. If you have something you want the player to +Use, put it in this list! */

							|| !Q_stricmp(viewing->classname,"item_doorknob")							
							|| (!Q_stricmp(viewing->classname,"monster_olive") && (viewing->style == 0))
							|| !Q_stricmp(viewing->classname,"monster_worker")
							|| (!Q_stricmp(viewing->classname,"item_pickable") && !(viewing->svflags & SVF_NOCLIENT))

							|| !Q_stricmp(viewing->classname,"item_suitcase")
							|| !Q_stricmp(viewing->classname,"item_wallbutton")
							|| !Q_stricmp(viewing->classname,"item_bottle")
							|| (!Q_stricmp(viewing->classname,"func_usebutton") && !(viewing->svflags & SVF_NOCLIENT))

							|| (!Q_stricmp(viewing->classname,"func_inspect")  && (viewing->count <= 0) )

							|| !Q_stricmp(viewing->classname,"item_pickup")
							|| !Q_stricmp(viewing->classname,"item_bulletbox")
							|| !Q_stricmp(viewing->classname,"item_deck")
							|| !Q_stricmp(viewing->classname,"item_radio")

							|| !Q_stricmp(viewing->classname,"target_commentary")

							


							//|| (!Q_stricmp(viewing->classname,"monster_npc") && viewing->target)
							//|| !Q_stricmp(viewing->classname,"item_pickup")
							//|| !Q_stricmp(viewing->classname,"item_comp")
						)
						&& (range <= 72)
					)
				  )
