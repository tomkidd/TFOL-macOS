...
//===================================
	...
	else
	{
		...
		if (ent->client->ps.stats[STAT_CAMTURN] > 0)
			angles[ROLL] = ent->client->ps.stats[STAT_CAMTURN];
...
//===================================
	...
	if (ent->client->ps.stats[STAT_CAMTURN] > 0)
	{
		v[2] -= ent->client->ps.stats[STAT_CAMTURN] *  1.35;
		v[0] += ent->client->ps.stats[STAT_CAMTURN] * 2;
	}

	if (ent->client->ps.stats[STAT_CAMDIP] > 0)
	{
		v[2] -= ent->client->ps.stats[STAT_CAMDIP] ; //dip down
	}
...
*/			
		...
		if (!deathmatch->value || !((int)dmflags->value & DF_NO_FALLING) )
		{
			if (ent->client->ps.stats[STAT_GODMODE] <= 0 && !ent->client->jetpack)
				T_Damage (ent, world, world, dir, ent->s.origin, vec3_origin, damage, 0, 0, MOD_FALLING);
		}
	}
	else if (delta > 15)
